...
coding = data2018$YearsCoding
procoding = data2018$YearsCodingProf
...
coding_df <- data.frame(Category = c(coding, procoding),
                        Type = c(rep("Usual coding", length(coding)), rep("Professional coding", length(procoding))))

coding_counts <- coding_df %>%
  group_by(Category, Type) %>%
  summarise(count = n(), .groups = "drop_last") %>%
  mutate(Category = factor(Category, levels = order))

plot_ly(coding_counts, x = ~Category, y = ~count, type = 'bar', color = ~Type, frame = ~Type,
        colors = palette,
...
inputPanel(
  selectInput("top_or_bottom_languages", "Ranking", c("Top 10", "Bottom 10")),
)
renderPlot({
do_top_languages= (input$top_or_bottom_languages == "Top 10")
mult_languages <- 1
ylim_languages <- 0.075
if(do_top_languages){
  mult_languages <- -mult_languages
  ylim_languages <- 0.75
}
...
if (do_top_languages) {
    languages_2018 <- languages_2018 %>% arrange(desc(freq)) %>% head(10)
    languages_2023 <- languages_2023 %>% arrange(desc(freq)) %>% head(10)
  } else {
    languages_2018 <- languages_2018 %>% arrange(freq) %>% head(10)
    languages_2023 <- languages_2023 %>% arrange(freq) %>% head(10)
  }

plot2018 <- ggplot(languages_2018, aes(x=reorder(LanguageWorkedWith, mult_languages*freq), y=freq)) +
  geom_bar(stat="identity", fill=palette[1]) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, ylim_languages, ylim_languages/3)) + 
  coord_cartesian(ylim = c(0, ylim_languages)) + 
  labs(title = "2018",
       x = "Language",
       y = "Proportion of developers using")

plot2023 <- ggplot(languages_2023, aes(x=reorder(LanguageWorkedWith, mult_languages*freq), y=freq)) +
...
grid.arrange(
  ...
  ncol=2,
  top=text_grob(
    paste(input$top_or_bottom_languages," programming languages in 2018 and 2023"), 
    color="black", 
    face="bold", 
    size=18
    )
...
inputPanel(
  selectInput("top_or_bottom_2018", "Ranking", c("Top 10", "Bottom 10")),
)
renderPlotly({
devtype_median_salary_2018 <- data2018 %>%
  ...
  drop_na() %>%
  group_by(DevType) %>%
  summarise(median_salary = round(median(ConvertedSalary, na.rm = TRUE),2))

do_top_2018 = (input$top_or_bottom_2018 == "Top 10")
mult_2018 <- 1
if(do_top_2018){
  mult_2018 <- -mult_2018
}
if (do_top_2018) {
    devtype_median_salary_2018 <- devtype_median_salary_2018 %>% arrange(desc(median_salary)) %>% head(10)
  } else {
    devtype_median_salary_2018 <- devtype_median_salary_2018 %>% arrange(median_salary) %>% head(10)
  }


plot_salary_2018 <- ggplot(devtype_median_salary_2018, 
                           aes(x=reorder(DevType, mult_2018*median_salary), 
                               y=median_salary,
...
inputPanel(
  selectInput("top_or_bottom_2023", "Ranking", c("Top 10", "Bottom 10")),
)
renderPlotly({
devtype_median_salary_2023 <- data2023 %>%
  ...
  drop_na() %>%
  group_by(DevType) %>%
  summarise(median_salary = round(median(ConvertedCompYearly, na.rm = TRUE),2))

do_top_2023 = (input$top_or_bottom_2023 == "Top 10")
mult_2023 <- 1
if(do_top_2023){
  mult_2023 <- -mult_2023
}
if (do_top_2023) {
    devtype_median_salary_2023 <- devtype_median_salary_2023 %>% arrange(desc(median_salary)) %>% head(10)
  } else {
    devtype_median_salary_2023 <- devtype_median_salary_2023 %>% arrange(median_salary) %>% head(10)
  }

plot_salary_2023 <- ggplot(devtype_median_salary_2023, 
                           aes(x=reorder(DevType, mult_2023*median_salary), 
                               y=median_salary,
                               text=paste('Yearly Median: $',formatC(median_salary, big.mark=',', format = 'f')))) +
  geom_bar(stat="identity", fill=palette[2]) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
